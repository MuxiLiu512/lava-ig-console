...
const PAGES = [
  ...
  { id: "pbsrc", file: "config/playbook-sources.md", name: "外部知識來源",
    sub: "系統每天去讀哪些人的曝光研究",
    why: "我們內部的學習迴路只吃自己 11 篇貼文的資料，那個樣本量任何結論都不顯著。" +
         "系統每天去這些網站讀「IG／Reels 怎麼做才有曝光」的研究，把帶數字的主張抓回來，" +
         "跟我們現在的做法比對，不一致的開成提案等你決定。" +
         "抓回來的是別人的主張不是事實，所以永遠不會自動改設定。" +
         "想加來源就加一行網址，建議加「自己做資料研究的」而不是「整理別人文章的」。" },
  { id: "vsrc", file: "config/video-sources.md", name: "影片片源",
    sub: "哪些頻道的畫面可以用，哪些要避開",
    why: "用 YouTube 截圖當底圖時照這份排序。新聞台的二次上傳會把自家台標與" +
         "中文字幕燒進畫面，那種圖鋪滿等於把別台浮水印印出去。" +
         "這是片源的問題不是哪一格的問題——同一支影片每一格都髒，所以要在選片就避開。" +
         "清單需要你補：看到又髒又常出現的頻道就加一行，存檔即生效。" },
