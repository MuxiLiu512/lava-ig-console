...
**相關檔案**
`scripts/make_reel.py`（現行對照組產生器，DUR_BODY 2.6 秒是本文第 0 節的檢驗對象，且輸出無音訊串流）
`scripts/ig_insights.py`（目前僅骨架，留存指標尚未接上，測量計畫要靠它）
`docs/design/風格規格-v1.0.md`（第五條禁句、第九條 mood 後綴、第三條 CTA 逐字鎖定，分鏡閘門直接沿用）