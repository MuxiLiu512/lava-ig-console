...
**關鍵檔案**
- `scripts/auto_render.sh`（:55-65 stash 復原反向，:100+ `git add -A` 過寬，:86 post-qa 觸發條件）
- `scripts/sync_console.py`（:128 `save()` 非原子、:1160 `alert` 無 token 靜默 return、:1275 `except` 吞掉 EDEADLK、:318-341 reconcile 只看 ClickUp）
- `scripts/forage_shots.py`（:646 `MIN_SCORE`、:651 `keep >= 2` 是單輪 per-slide 上限、:27 CURATOR_URL 打 n8n）
- `docs/app.js`（:57-85 CAS 正確、:91-98 setImg 走 GitHub blob、:303 CTA 無候選是設計、:509 排程按鈕前置、:522 取消排程降級無回程）
- `data/archived-posts.json`（兩篇過期 `scheduled`：weak-ties、已讀不回-v5）
- `排版引擎/render_post_v5.py`（不在版控）